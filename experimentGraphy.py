...
def stateGraph():
	...
	for i in range(2,12):
		...
		x_axis.append(i)
		for j in range(4):
			DfaSearchSim.NUM_STATES = i
			num = 200
			accuracyList = DfaSearchSim.sim2(num)
			randomDrawnAccuracy = np.mean(accuracyList)
			y.append(randomDrawnAccuracy)
			# print marker to see progress of program
			print("--------------------------------------")
			print('Finish states', i,' try ', j)
			print("--------------------------------------")
